...
def inception_fid( imgs, batch_size, device=None):
    """Computes the inception fid of the generated images imgs
    """
    # x_real_fid = x_real_fid / 2 + 0.5
    # imgs = imgs / 2 + 0.5
    ...
    mu_fake, sigma_fake = calculate_activation_statistics(
        imgs, inception_model, batch_size, device=device
        )
    path='./fid_stats.npz'
    f = np.load(path)
    ...
    #     x_real_fid, inception_model, batch_size, device=device
    #     )
    # np.savez_compressed('./fid_stats.npz', mu=mu_real, sigma=sigma_real)
