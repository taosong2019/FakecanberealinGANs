...
class Generator(nn.Module):
    def __init__(self, z_dim, nlabels, size, embed_size=256, nfilter=64, **kwargs):
        ...
        self.fc = nn.Linear(z_dim + embed_size, 16 * 64 * 4 * 4)

        self.resnet_0_0 = ResnetBlock(16 * nf, 16 * nf)
        self.resnet_0_1 = ResnetBlock(16 * nf, 16 * nf)

        self.resnet_1_0 = ResnetBlock(16 * nf, 8 * nf)
        self.resnet_1_1 = ResnetBlock(8 * nf, 8 * nf)

        self.resnet_2_0 = ResnetBlock(8 * nf, 4 * nf)
        self.resnet_2_1 = ResnetBlock(4 * nf, 4 * nf)

        self.resnet_3_0 = ResnetBlock(4 * nf, 2 * nf)
        self.resnet_3_1 = ResnetBlock(2 * nf, 2 * nf)

        self.resnet_4_0 = ResnetBlock(2 * nf, 1 * nf)
        self.resnet_4_1 = ResnetBlock(1 * nf, 1 * nf)
    ...
    def forward(self, z, y):
        ...
        out = self.fc(yz)
        out = out.view(batch_size, 16*self.nf, 4, 4)
...
class Discriminator(nn.Module):
    def __init__(self, z_dim, nlabels, size, embed_size=256, nfilter=64, **kwargs):
        ...
        self.fc = nn.Linear(16*nf*4*4, nlabels)
