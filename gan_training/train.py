...
class Trainer(object):
    ...
    def discriminator_trainstep(self, x_real, y, z):
        ...
        z_real = z[64:320, :]
        y_real = torch.cat([y, y, y, y], 0)
        z = z[0:64, :]
        with torch.no_grad():
            x_real2 = self.generator(z_real, y_real)
            _, kindex = torch.topk(-self.discriminator(x_real2, y_real), 32, dim=0)
            kindex = kindex.squeeze()
            # print(kindex)
            # print(kindex.size())
            x_real2 = x_real2[kindex]
        x_real[32:64, :, :, :] = x_real2
